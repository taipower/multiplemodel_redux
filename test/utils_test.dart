...
void main() {
  group('$Utils', () {
    ...
    Product item = new Product(new DateTime.now(),'test 1',98.0,10,'');
    item.key = '1';
    listProduct.add(item);
    item = new Product(new DateTime.now(), 'test 2', 34.75,30,'');
    item.key = '2';
    listProduct.add(item);
    item = new Product(new DateTime.now(), 'test 3', 56.0, 100,'');
    item.key = '3';
    listProduct.add(item);
    item = new Product(new DateTime.now(), 'test 4', 45.0, 74,'');
    item.key = '4';
    listProduct.add(item);
    item = new Product(new DateTime.now(), 'test 5', 67.0, 85,'');
    item.key = '5';
    listProduct.add(item);
    item = new Product(new DateTime.now(), 'test 6', 32.0, 7,'');
    item.key = '6';
    listProduct.add(item);
    item = new Product(new DateTime.now(), 'test 7', 76.25, 35,'');
    item.key = '7';
    listProduct.add(item);
    item = new Product(new DateTime.now(), 'test 8', 765.0, 234,'');
    item.key = '8';
    listProduct.add(item);
    item = new Product(new DateTime.now(), 'test 9', 376.0, 157,'');
    item.key = '9';
    listProduct.add(item);
    item = new Product(new DateTime.now(), 'test 10', 546.50, 43,'');
    item.key = '10';
    listProduct.add(item);

    List<Product> cart = new List();
    Product cutoffItem = new Product(new DateTime.now(), 'test 4', 45.0, 20,'');
    cutoffItem.key = '4';
    cart.add(cutoffItem);
    cutoffItem = new Product(new DateTime.now(), 'test 6', 32.0, 2,'');
    cutoffItem.key = '6';
    cart.add(cutoffItem);
    cutoffItem = new Product(new DateTime.now(), 'test 9', 376.0, 100,'');
    cutoffItem.key = '9';
    cart.add(cutoffItem);
    ...
    test('test cutoffStock', (){
      List<Product> productTest = new List();
